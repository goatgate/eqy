...
if grep "SAT temporal induction proof finished - model found for base case: FAIL!" run.log > /dev/null ; then
	echo FAIL > status
	echo "Could not prove equivalence of partition 'spm.p' using strategy 'sat'"
	...
	echo PASS > status
	echo "Proved equivalence of partition 'spm.p' using strategy 'sat'"
else
	echo ERROR > status
	echo "Execution of strategy 'sat' on partition 'spm.p' encountered an error.
Details can be found in 'spm/strategies/spm.p/sat/run.log'."
	exit 1
fi
exit 0
